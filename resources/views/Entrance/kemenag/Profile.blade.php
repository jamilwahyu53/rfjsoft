...
<div class="container h-100 ">
    <div class="row h-100 justify-content-md-center align-items-center m-3" style="min-height: 100vh;">
        <div class="col-12 mb-2">
            <button class="btn btn-danger btn-sm float-end" @click="resetPass">RESET PASSWORD</button>
        </div>
            ...
            <form id="register-form" @submit.prevent="saveAndSubmit">
                ...
                <div class="row">
                    <div class="col-12">
                        ...
                        <button class="btn btnToska w-100 mt-2" :disabled="loading" >UPDATE</button>
                    </div>
    ...
    @include('EntranceComponents.ModalResetPass', ['id' => 'modalUpdatePass', 'title' => 'Reset Password'])
...
createApp({
    ...
    setup() {
        ...
        const code_user_login = ref("{{ data_get($userLogin['data'], 'code', data_get($userLogin['data'], 'Kode', '')) }}");
        //const code_user_login = ref("00bca31f-b594-4ecc-b561-68c8fe56904a");
        ...
        const firstLoadDtReg = async () => {
            ...
            const form_profile = {code : code_user_login.value };
            const resDataCust = await ApiService.post('/GetProfile', form_profile);
            if(resDataCust.status){
            
                await getSatker(resDataCust.data.organization);

                Object.assign(form_register, resDataCust.data);

            } else {
                message_modal.value = resDataCust.message;
                $("#modalDefault").modal("show");
            }
        };
        ...
        const saveAndSubmit = async () => {
            if (validateForm()) {   
                ...
                const res = await ApiService.post('/UpdateDataVisitor', form_register);
                if(res.status){
                    afterReg.value = true;
                    message_modal.value = "Update Berhasil";
                   $("#modalDefault").modal("show");
